...
const rn = require('./register_names');
const rv = require('./register_values');

class RegisterDefinition {
  //Base class for register definitions.
  constructor(register, length, writeable=false, readable=true) {
  ...
  encode(data) {
    console.log("Implement me (enc)");
    //throw NotImplementedError()
  }

  decode(decoder) {
    console.log("Implement me (dec)");
    //throw NotImplementedError()¨
  }
}

class StringRegister extends RegisterDefinition {
  //A string register.
  constructor(register, length, writeable=false, readable=true){
    super(register, length, writeable, readable)
  }

  decode(decoder) {
    console.log("Decoding: ", decoder)
    try {
      return  new Buffer.from(decoder).toString();
    }
    catch (error) {
...
class NumberRegister extends RegisterDefinition {
  ...
  constructor (
    ...
    invalid_value=null,
        ...
        this._decode_function_name = decode_function_name
        this._encode_function_name = encode_function_name
        this._invalid_value = invalid_value
    }

    decode(decoder) {
        result = decoder[this._decode_function_name];

        if (this._invalid_value !== null && result == this._invalid_value) {
          return null;
        }

        if (this.gain !== 1) {
          result = result/this.gain;
        }
        ...
        if (typeof(this.unit) === 'function') {
          try {
            ...
            console.log("ERROR:  NumberRegister.decoder function");
            console.log(error.toString());
            //ValueError as err:
            //raise DecodeError from err
          }
        }
        else if (typeof(this.unit) === 'object') { //todo
          try {
            result = this.unit[result]
          }
          catch (error) {
            console.log("ERROR:  NumberRegister.decoder object");
            console.log(error.toString());
...
class U16Register extends NumberRegister {
  //Unsigned 16-bit register

  constructor(unit, gain, register, length, writeable=false, readable=true, ignore_invalid=false) {
    super(
      unit,
      gain,
      register,
      ...
      writeable=writeable,
      readable=readable,
      invalid_value = !ignore_invalid ? 2**16 : null)
  }
}


class U32Register extends NumberRegister {
  //Unsigned 32-bit register
  constructor(unit, gain, register, length, writeable=false) {
    super(
      unit,
      gain,
      register,
...
class I16Register extends NumberRegister {
...
class I32Register extends NumberRegister {
...
class I32AbsoluteValueRegister extends NumberRegister {
  ...
  // as a positive number, but are (in some cases) being reported as a negative number.
...
REGISTERS = {
  [rn.MODEL_NAME] : new StringRegister(30000, 15),
  [rn.SERIAL_NUMBER]: new StringRegister(30015, 10),
  [rn.ACCUMULATED_YIELD_ENERGY]: new U32Register("kWh", 100, 32106, 2),                               // Total KWh, ackumluated
  [rn.DAILY_YIELD_ENERGY]: new U32Register("kWh", 100, 32114, 2)                                // Day
}

/*
REGISTERS: dict[str, RegisterDefinition] = {total
    ...
    rn.ACCUMULATED_YIELD_ENERGY: U32Register("kWh", 100, 32106, 2),                               // Total KWh, ackumluated
    rn.DAILY_YIELD_ENERGY: U32Register("kWh", 100, 32114, 2),                                     // Day
...
module.exports = { REGISTERS, StringRegister }